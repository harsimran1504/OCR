...
while True:
    ...
    # showData = cv2.resize(showData, (160, 120))
    # cv2.imshow('receiver', showData)

########### EASYOCR ###########

    grayscale = cv2.cvtColor(showData, cv2.COLOR_BGR2GRAY)
    start_time = get_time_ms()
    result = reader.readtext(image=grayscale, canvas_size=150)
    print("Time taken: " + str(get_time_ms() - start_time) + "ms")
    text = ""
